...
function renderChat(){
  ...
  // ВАЖЛИВО: попередня спроба ховати навбар через display:none іноді
  // залишала його прихованим назавжди (клавіатура не закривалась, вийти
  // з чату можна було лише перезаходом) — тому навбар більше НІКОЛИ
  // примусово не ховається, навігація завжди лишається доступною.
  // Клавіатура закривається звичним чином — тапом по списку повідомлень
  // (стандартна поведінка чатів), а не автоматично.
  const messages = document.getElementById("chatMessages");
  const input = document.getElementById("chatInput");
  messages.addEventListener("click", () => { input.blur(); });
}
